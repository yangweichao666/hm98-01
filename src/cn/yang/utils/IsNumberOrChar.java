...
public class IsNumberOrChar {
    ...
    public List<Map> GetStringAndSortNum(String[] list) {
        ...
        for (int i = 0; i < list.length; i++) {
            ...
            //word代表需要进行处理判断的字
            word = list[i];
            //判断word是不是空的串，因为有可能根据空格分割的串中有空的换行符或者空串，不进行处理//其实在MAin类的Divide方法已将按空格分 只是按单个空格
            //为了错误处理
                ...
                if (firstChar == 'w' || firstChar == 'i' || firstChar == 'b' || firstChar == 'd' || firstChar == 'e' || firstChar == 't') {
                    ...
                    //不是关键字，但是包含关键字的前一个字符串
                        ...
                        if (IsID(word)) {
                            ...
                            err.append("这个" + word + "不是合法的ID字符，所在的位置在：第" + (i + 1) + "个单词");
                        }
                    }
            ...
            if (cType == "digit") {
                if (IsNum(word)) {
                    ...
                    err.append("这个" + word + "不是合法的NUM字符，所在的位置在：第" + (i + 1) + "个单词");
                }
            }
            //首字符为其他字符
            if (cType == "opts") {
    ...
    /**
     * 判断一个字符是不是合法的ID,如果是，则返回true，否则，返回这个word的错误码false//错误处理
     *
     * @param word
     * @return boolean
     */
